...
struct sesslist {
    int nsessions;
    TCHAR **sessions;
    TCHAR *buffer;		       /* so memory can be freed later */
};
...
struct backend_tag {
    const TCHAR *(*init) (void *frontend_handle, void **backend_handle,
			 Config *cfg,
 ...
 */
extern const TCHAR *const appname;
...
struct config_tag {
    ...
    int compose_key;
    int ctrlaltkeys;
    TCHAR wintitle[256];		       /* initial window title */
    /* Terminal options */
    int savelines;
    int dec_om;
    int wrap_mode;
...
#endif
void set_title(void *frontend, TCHAR *);
void set_icon(void *frontend, TCHAR *);
...
#ifdef _UNICODE
#define _write_tclip write_clip
#else
#define _write_tclip write_aclip
#endif

void get_clip(void *frontend, wchar_t **, int *);
void optimised_move(void *frontend, int, int, int);
void set_raw_mouse_mode(void *frontend, int);
void connection_fatal(void *frontend, TCHAR *, ...);
void fatalbox(TCHAR *, ...);
void modalfatalbox(TCHAR *, ...);
#ifdef macintosh
...
void logwevent(void *frontend, const wchar_t *);
 ...
 * cmdline.c).
 */
int cmdline_process_param(TCHAR *, TCHAR *, int, Config *);
