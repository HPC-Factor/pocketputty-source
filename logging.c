...
void log_eventlog(void *handle, const char *event)
{
	...
	logfopen(ctx);
    if (ctx->lgfp)
	_ftprintf(ctx->lgfp, _T("Event Log: %s\r\n"), event);
}
		...
		char *texttype, void *data, int len)
{
	...
	logfopen(ctx);
    if (ctx->lgfp) {
	_ftprintf(ctx->lgfp, _T("%s packet type %d / 0x%02x (%s)\r\n"),
		direction == PKT_INCOMING ? _T("Incoming") : _T("Outgoing"),
		type, type, texttype);
	for (i = 0; i < len; i += 16) {
	    _stprintf(dumpdata, _T("  %08x%*s\r\n"), i, 1+3*16+2+16, _T(""));
	    for (j = 0; j < 16 && i+j < len; j++) {
		int c = ((unsigned char *)data)[i+j];
		_stprintf(smalldata, _T("%02x"), c);
		dumpdata[10+2+3*j] = smalldata[0];
		dumpdata[10+2+3*j+1] = smalldata[1];
		dumpdata[10+1+3*16+2+j] = (isprint(c) ? c : _T('.'));
	    }
	    _tcscpy(dumpdata + 10+1+3*16+2+j, _T("\r\n"));
	    _fputts(dumpdata, ctx->lgfp);
	}
	fflush(ctx->lgfp);
    }
}
...
void logfopen(void *handle)
{
	...
	if (i == 1)
	    writemod[0] = _T('a');	       /* set append mode */
	...
	sprintf(buf, "%s session log (%s mode) to file: ",
		(writemod[0] == _T('a')) ? "Appending" : "Writing new",
