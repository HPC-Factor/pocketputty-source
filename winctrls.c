...
HWND doctl(struct ctlpos *cp, RECT r,
	   TCHAR *wclass, int wstyle, int exstyle, TCHAR *wtext, int wid)
{
    HWND ctl;
    /*
 ...
 */
void bartitle(struct ctlpos *cp, TCHAR *name, int id)
{
    RECT r;
    ...
    doctl(cp, r, _T("STATIC"), WS_CHILD | WS_VISIBLE, 0, name, id);
}

/*
 * Begin a grouping box, with or without a group title.
 */
void beginbox(struct ctlpos *cp, TCHAR *name, int idbox)
{
...
void endbox(struct ctlpos *cp)
{
    ...
    doctl(cp, r, _T("BUTTON"), BS_GROUPBOX | WS_CHILD | WS_VISIBLE, 0,
	  cp->boxtext ? cp->boxtext : _T(""), cp->boxid);
    cp->ypos += GAPYBOX;
}
...
void multiedit(struct ctlpos *cp, int password, ...)
{
	...
	r.top = cp->ypos;
	r.bottom = STATICHEIGHT;
	doctl(cp, r, _T("STATIC"), WS_CHILD | WS_VISIBLE, 0, text, staticid);
	r.top = cp->ypos + 8 + GAPWITHIN;
	r.bottom = EDITHEIGHT;
	doctl(cp, r, _T("EDIT"),
	      WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_AUTOHSCROLL |
	      (password ? ES_PASSWORD : 0),
	      WS_EX_CLIENTEDGE, _T(""), editid);
    }
    va_end(ap);
 ...
 */
void combobox(struct ctlpos *cp, TCHAR *text, int staticid, int listid)
{
    RECT r;
    ...
    doctl(cp, r, _T("COMBOBOX"),
	  WS_CHILD | WS_VISIBLE | WS_TABSTOP | WS_VSCROLL |
	  CBS_DROPDOWN | CBS_HASSTRINGS, WS_EX_CLIENTEDGE, _T(""), listid);

    cp->ypos += STATICHEIGHT + GAPWITHIN + COMBOHEIGHT + GAPBETWEEN;
}

struct radio { TCHAR *text; int id; };

static void radioline_common(struct ctlpos *cp, TCHAR *text, int id,
			     int nacross, struct radio *buttons, int nbuttons)
{
	...
	doctl(cp, r, _T("STATIC"), WS_CHILD | WS_VISIBLE, 0, text, id);
    }

    group = WS_GROUP;
    i = 0;
    for (j = 0; j < nbuttons; j++) {
	TCHAR *btext = buttons[j].text;
 ...
 */
void radioline(struct ctlpos *cp, TCHAR *text, int id, int nacross, ...)
{
    va_list ap;
	...
	buttons[i].text = va_arg(ap, TCHAR *);
 ...
 * them.
 */
void radiobig(struct ctlpos *cp, TCHAR *text, int id, ...)
{
    va_list ap;
    struct radio *buttons;
    int i, nbuttons;
 ...
 */
void checkbox(struct ctlpos *cp, TCHAR *text, int id)
{
    RECT r;
 ...
 */
TCHAR *staticwrap(struct ctlpos *cp, HWND hwnd, TCHAR *text, int *lines)
{
    HDC hdc = GetDC(hwnd);
    ...
    ret = snewn(1+_tcsclen(text), TCHAR);
    p = text;
    q = ret;
    pwidths = snewn(1+_tcsclen(text), INT);
    ...
    while (*p) {
	if (!GetTextExtentExPoint(hdc, p, _tcsclen(p), width,
				  &nfit, pwidths, &size) ||
	    (size_t)nfit >= _tcsclen(p)) {
	    /*
	 ...
	 */
	for (j = nfit; j > 0; j--) {
	    if (_istspace((_unsigned_tchar) p[j])) {
		nfit = j;
		break;
	    }
	}
	...
	p += nfit;
	while (*p && _istspace((_unsigned_tchar)*p))
	    p++;
 ...
 */
void statictext(struct ctlpos *cp, TCHAR *text, int lines, int id)
{
    RECT r;
...
void paneltitle(struct ctlpos *cp, int id)
{
    ...
    cp->ypos += r.bottom + GAPBETWEEN;
    doctl(cp, r, _T("STATIC"), WS_CHILD | WS_VISIBLE 
#ifndef _WIN32_WCE
	| SS_OWNERDRAW
#endif
	 , 0, NULL, id);
 ...
 */
void staticbtn(struct ctlpos *cp, TCHAR *stext, int sid,
	       TCHAR *btext, int bid)
{
    ...
    r.right = lwid;
    r.bottom = STATICHEIGHT;
    doctl(cp, r, _T("STATIC"), WS_CHILD | WS_VISIBLE, 0, stext, sid);
 ...
 */
void button(struct ctlpos *cp, TCHAR *btext, int bid, int defbtn)
{
    RECT r;
 ...
 */
void static2btn(struct ctlpos *cp, TCHAR *stext, int sid,
		TCHAR *btext1, int bid1, TCHAR *btext2, int bid2)
{
    const int height = (PUSHBTNHEIGHT > STATICHEIGHT ?
			PUSHBTNHEIGHT : STATICHEIGHT);
    RECT r;
 ...
 */
static void staticedit_internal(struct ctlpos *cp, TCHAR *stext,
				...
				int style)
{
    ...
    doctl(cp, r, _T("EDIT"),
	  ...
	  WS_EX_CLIENTEDGE, _T(""), eid);

    cp->ypos += height + GAPBETWEEN;
}

void staticedit(struct ctlpos *cp, TCHAR *stext,
		int sid, int eid, int percentedit)
{
    staticedit_internal(cp, stext, sid, eid, percentedit, 0);
}

void staticpassedit(struct ctlpos *cp, TCHAR *stext,
		    int sid, int eid, int percentedit)
{
 ...
 * its left.
 */
void staticddl(struct ctlpos *cp, TCHAR *stext,
	       int sid, int lid, int percentlist)
{
    ...
    doctl(cp, r, _T("COMBOBOX"),
	  ...
	  CBS_DROPDOWNLIST | CBS_HASSTRINGS, WS_EX_CLIENTEDGE, _T(""), lid);
 ...
 */
void staticcombo(struct ctlpos *cp, TCHAR *stext,
		 int sid, int lid, int percentlist)
{
    const int height = (COMBOHEIGHT > STATICHEIGHT ?
			COMBOHEIGHT : STATICHEIGHT);
    ...
    doctl(cp, r, _T("COMBOBOX"),
	  WS_CHILD | WS_VISIBLE | WS_TABSTOP | WS_VSCROLL |
	  CBS_DROPDOWN | CBS_HASSTRINGS, WS_EX_CLIENTEDGE, _T(""), lid);
 ...
 */
void staticddlbig(struct ctlpos *cp, TCHAR *stext,
		  int sid, int lid)
{
    RECT r;
 ...
 */
void bigeditctrl(struct ctlpos *cp, TCHAR *stext,
 ...
 */
void listbox(struct ctlpos *cp, TCHAR *stext,
 ...
 */
void editbutton(struct ctlpos *cp, TCHAR *stext, int sid,
		int eid, TCHAR *btext, int bid)
{
...
void prefslist(struct prefslist *hdl, struct ctlpos *cp, int lines,
	       TCHAR *stext, int sid, int listid, int upbid, int dnbid)
{
 ...
 * string.
 */
static TCHAR *shortcut_escape(TCHAR *text, TCHAR shortcut)
{
    TCHAR *ret;
    TCHAR *p, *q;
    ...
    ret = snewn(2*_tcsclen(text)+1, TCHAR);   /* size potentially doubles! */
    shortcut = _totlower((_unsigned_tchar)shortcut);

    p = text;
    q = ret;
    while (*p) {
	if (shortcut != NO_SHORTCUT &&
	    _totlower((_unsigned_tchar)*p) == shortcut) {
	    *q++ = _T('&');
...
void winctrl_add_shortcuts(struct dlgparam *dp, struct winctrl *c)
{
	...
	if (c->shortcuts[i] != NO_SHORTCUT) {
	    _unsigned_tchar s = _totlower((_unsigned_tchar)c->shortcuts[i]);
		    ...
		    struct ctlpos *cp, struct controlset *s, int *id)
{
    ...
    TCHAR shortcuts[MAX_SHORTCUTS_PER_CTRL];
    int nshortcuts;
    TCHAR *escaped;
	...
	switch (ctrl->generic.type) {
	  ...
	  case CTRL_LISTBOX:
		...
		shortcuts[nshortcuts++] = _T('u');   /* Up */
		shortcuts[nshortcuts++] = _T('d');   /* Down */
	  ...
	  case CTRL_FILESELECT:
	    ...
	    editbutton(&pos, escaped, base_id, base_id+1,
		       _T("Bro&wse..."), base_id+2);
	    shortcuts[nshortcuts++] = _T('w');
	    sfree(escaped);
	    break;
	  case CTRL_FONTSELECT:
	    ...
	    staticbtn(&pos, _T(""), base_id+1, _T("Change..."), base_id+2);
	    sfree(escaped);
	    data = snew(FontSpec);
	    break;
	  default:
			   ...
			   WPARAM wParam, LPARAM lParam)
{
	...
	if (id == 2 &&
	    ...
	    _tcsnccpy(lf.lfFaceName, fs.name,
		    sizeof(lf.lfFaceName) - 1); warning
	    lf.lfFaceName[sizeof(lf.lfFaceName) / sizeof(TCHAR) - 1] = _T('\0');
	    ...
	    if (ChooseFont(&cf)) {
		_tcsnccpy(fs.name, lf.lfFaceName,
			sizeof(fs.name) / sizeof(TCHAR) - 1); warning
		fs.name[sizeof(fs.name) / sizeof(TCHAR) - 1] = _T('\0'); warning
...
int winctrl_context_help(struct dlgparam *dp, HWND hwnd, int id)
{
    ...
    cmd = _duptprintf(_T("JI(`',`%s')"), c->ctrl->generic.helpctx.p);
...
void dlg_editbox_set(union control *ctrl, void *dlg, TCHAR const *text)
{
    struct dlgparam *dp = (struct dlgparam *)dlg;
    struct winctrl *c = dlg_findbyctrl(dp, ctrl);
...
void dlg_editbox_get(union control *ctrl, void *dlg, TCHAR *buffer, int length)
{
    struct dlgparam *dp = (struct dlgparam *)dlg;
    struct winctrl *c = dlg_findbyctrl(dp, ctrl);
    assert(c && c->ctrl->generic.type == CTRL_EDITBOX);
...
void dlg_listbox_add(union control *ctrl, void *dlg, TCHAR const *text)
{
    struct dlgparam *dp = (struct dlgparam *)dlg;
    struct winctrl *c = dlg_findbyctrl(dp, ctrl);
    int msg;
...
void dlg_text_set(union control *ctrl, void *dlg, TCHAR const *text)
{
    struct dlgparam *dp = (struct dlgparam *)dlg;
    struct winctrl *c = dlg_findbyctrl(dp, ctrl);
...
#ifdef _UNICODE
    fn_path = ansi2unicode(fn.path);
    SetDlgItemText(dp->hwnd, c->base_id+1, fn_path);
    sfree(fn_path);
#else
...
{
#ifdef _UNICODE
    wchar_t fn_path[FILENAME_MAX];
#endif
    struct dlgparam *dp = (struct dlgparam *)dlg;
    struct winctrl *c = dlg_findbyctrl(dp, ctrl);
    assert(c && c->ctrl->generic.type == CTRL_FILESELECT);
#ifdef _UNICODE
    GetDlgItemText(dp->hwnd, c->base_id+1, fn_path, lenof(fn_path));
    fn_path[lenof(fn_path)-1] = L'\0';
    wcstombs(fn->path, fn_path, lenof(fn->path));
#else
    GetDlgItemText(dp->hwnd, c->base_id+1, fn->path, lenof(fn->path));
    fn->path[lenof(fn->path)-1] = _T('\0');
#endif
}
...
void dlg_fontsel_set(union control *ctrl, void *dlg, FontSpec fs)
{
    ...
    *(FontSpec *)c->data = fs;	       /* structure copy */
    ...
    boldstr = (fs.isbold ? _T("bold, ") : _T(""));
    if (fs.height == 0)
	buf = _duptprintf(_T("Font: %s, %sdefault height"), fs.name, boldstr);
    else
	buf = _duptprintf(_T("Font: %s, %s%d-point"), fs.name, boldstr,
...
void dlg_error_msg(void *dlg, TCHAR *msg)
{
    struct dlgparam *dp = (struct dlgparam *)dlg;
