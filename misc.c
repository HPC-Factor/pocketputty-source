...
void mlog(char *file, int line)
{
    ...
    if (!fp) {
#ifdef _IPAQ
	fp = fopen("\\My Documents\\putty_mem.log", "w");
#elif _WIN32_WCE
	fp = fopen("\\Storage Card\\putty_mem.log", "w");
#else
...
#ifdef MALLOC_LOG
#ifdef _UNICODE
	wchar_t *mlogfile = malloc((strlen(mlog_file) + 1) * sizeof(TCHAR));
	mlogfile[strlen(mlog_file)] = L'\0';
	mbstowcs(mlogfile, mlog_file, strlen(mlog_file));
	swprintf(str, _T("Out of memory! (%s:%d, size=%d)"),
		mlog_file, mlog_line, size);
	free(mlogfile);
#else
	_stprintf(str, _T("Out of memory! (%s:%d, size=%d)"),
		mlog_file, mlog_line, size);
#endif
	fprintf(fp, "*** %s\n", str);
	fclose(fp);
