...
static void c_write1(Telnet telnet, int c)
{
    ...
    char cc = (char) c;
#ifdef DEBUG
TCHAR *wbuf = snew(TCHAR);
mbstowcs(wbuf, &cc, 1);
_debug_tprintf((_T("TNc_write1(\"%c\");"), c));
sfree(wbuf);
#endif
...
static int telnet_closing(Plug plug, const TCHAR *error_msg, int error_code,
			  int calling_back)
{
	...
	_logtevent(telnet->frontend, error_msg);
	connection_fatal(telnet->frontend, _T("%s"), error_msg);
 ...
 * freed by the caller.
 */
static const TCHAR *telnet_init(void *frontend_handle, void **backend_handle,
			       Config *cfg,
