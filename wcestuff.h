/*
 * wcestuff.h: Windows CE-specific inter-module stuff.
 *
 * winstuff.h modification by Ales Berka
 */
...
// Temporary - to be proessed later in special section
//Aleq
...
#define TNULL MAKEWORD(0, 0)

// No help on Windows CE
...
#define _tf_open(filename, mode) ( _tfopen((filename).path, (mode)) )
...
#define PUTTY_REG_POS _T("Software\\SimonTatham\\PuTTY")
#define PUTTY_REG_PARENT _T("Software\\SimonTatham")
#define PUTTY_REG_PARENT_CHILD _T("PuTTY")
#define PUTTY_REG_GPARENT _T("Software")
#define PUTTY_REG_GPARENT_CHILD _T("SimonTatham")
 ...
 */
#define FILTER_KEY_FILES (_T("PuTTY Private Key Files (*.ppk)\0*.ppk\0All Files (*.*)\0*\0\0\0"))
#define FILTER_WAVE_FILES (_T("Wave Files (*.wav)\0*.WAV\0" \
			       _T("All Files (*.*)\0*\0\0\0")))
 ...
 */
void split_into_argv(TCHAR *, int *, TCHAR ***, TCHAR ***);
...
struct dlgparam {
    ...
    int nctrltrees;
    TCHAR *wintitle;		       /* title of actual window */
    TCHAR *errtitle;		       /* title of error sub-messageboxes */
    void *data;			       /* data to pass in refresh events */
    union control *focused, *lastfocused; /* which ctrl has focus now/before */
    TCHAR shortcuts[128];	       /* track which shortcuts in use */
...
void sesssaver(struct ctlpos *cp, TCHAR *text,
	       int staticid, int editid, int listid, ...);
void envsetter(struct ctlpos *cp, TCHAR *stext, int sid,
	       TCHAR *e1stext, int e1sid, int e1id,
	       TCHAR *e2stext, int e2sid, int e2id,
	       int listid, TCHAR *b1text, int b1id, TCHAR *b2text, int b2id);
void charclass(struct ctlpos *cp, TCHAR *stext, int sid, int listid,
	       TCHAR *btext, int bid, int eid, TCHAR *s2text, int s2id);
void colouredit(struct ctlpos *cp, TCHAR *stext, int sid, int listid,
		TCHAR *btext, int bid, ...);
...
void fwdsetter(struct ctlpos *cp, int listid, TCHAR *stext, int sid,
	       TCHAR *e1stext, int e1sid, int e1id,
	       TCHAR *e2stext, int e2sid, int e2id,
	       TCHAR *btext, int bid,
	       TCHAR *r1text, int r1id, TCHAR *r2text, int r2id);
