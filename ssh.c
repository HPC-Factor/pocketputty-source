...
static const TCHAR *const ssh2_disconnect_reasons[] = {
    NULL,
    _T("SSH_DISCONNECT_HOST_NOT_ALLOWED_TO_CONNECT"),
    _T("SSH_DISCONNECT_PROTOCOL_ERROR"),
    _T("SSH_DISCONNECT_KEY_EXCHANGE_FAILED"),
    _T("SSH_DISCONNECT_HOST_AUTHENTICATION_FAILED"),
    _T("SSH_DISCONNECT_MAC_ERROR"),
    _T("SSH_DISCONNECT_COMPRESSION_ERROR"),
    _T("SSH_DISCONNECT_SERVICE_NOT_AVAILABLE"),
    _T("SSH_DISCONNECT_PROTOCOL_VERSION_NOT_SUPPORTED"),
    _T("SSH_DISCONNECT_HOST_KEY_NOT_VERIFIABLE"),
    _T("SSH_DISCONNECT_CONNECTION_LOST"),
    _T("SSH_DISCONNECT_BY_APPLICATION"),
    _T("SSH_DISCONNECT_TOO_MANY_CONNECTIONS"),
    _T("SSH_DISCONNECT_AUTH_CANCELLED_BY_USER"),
    _T("SSH_DISCONNECT_NO_MORE_AUTH_METHODS_AVAILABLE"),
    _T("SSH_DISCONNECT_ILLEGAL_USER_NAME"),
};
...
#ifdef _UNICODE
#define __logtevent(s) logwevent(ssh->frontend, s)
#define _logteventf logweventf
#else
#define __logtevent(s) logevent(ssh->frontend, s)
#define _logteventf logeventf
#endif

#define _logevent(s) logevent(ssh->frontend, s)
#define _logwevent(s) logwevent(ssh->frontend, s)
...
static void logweventf(Ssh ssh, const wchar_t *fmt, ...) {
	va_list ap;
    wchar_t *buf;
...
#define bombout(msg) \
    do { \
        TCHAR *text = _duptprintf msg; \
	ssh_do_close(ssh); \
        __logtevent(text); \
        connection_fatal(ssh->frontend, _T("%s"), text); \
        sfree(text); \
    } while (0)
  ...
  next_packet:
	...
	*datalen -= st->chunk;
    ...
    if (ssh->cipher && detect_attack(ssh->crcda_ctx, ssh->pktin.data,
				     st->biglen, NULL)) {
        bombout((_T("Network attack (CRC compensation) detected!")));
        crStop(0);
    }
	...
	bombout((_T("Incorrect CRC received on packet")));
	crStop(0);
    }
	...
	if (!zlib_decompress_block(ssh->sc_comp_ctx,
				   ...
				   &decompblk, &decomplen)) {
	    bombout((_T("Zlib decompression encountered invalid data")));
	    crStop(0);
	}
	...
	if (stringlen + 4 != ssh->pktin.length) {
	    bombout((_T("Received data packet with bogus string length")));
	    crStop(0);
	}
    }
...
#ifdef _UNICODE
	{
	    wchar_t *buf_nowlen = ansi2unicode(buf + nowlen);
	    bombout((_T("Server sent disconnect message:\n\"%s\""), buf_nowlen));
	    sfree(buf_nowlen);
	}
#else
...
static int ssh2_rdpkt(Ssh ssh, unsigned char **data, int *datalen)
{
  ...
  next_packet:
	...
	bombout((_T("Incoming packet was garbled on decryption")));
	crStop(0);
    }
	...
	bombout((_T("Incorrect MAC received on packet")));
	crStop(0);
    }
      ...
      case SSH2_MSG_DISCONNECT:
	    ...
	    int nowlen;
            ...
            if (reason > 0 && reason < lenof(ssh2_disconnect_reasons)) {
                buf = _duptprintf(_T("Received disconnect message (%s)"),
				ssh2_disconnect_reasons[reason]);
            } else {
                buf = _duptprintf(_T("Received disconnect message (unknown")
				  _T(" type %d)"), reason);
            }
            __logtevent(buf);
	    sfree(buf);
            buf = _duptprintf(_T("Disconnection message text: %n%.*s"),
			    &nowlen, msglen, ssh->pktin.data + 14);
            __logtevent(buf);
            bombout((_T("Server sent disconnect message\ntype %d (%s):\n\"%s\""),
                     reason,
                     (reason > 0 && reason < lenof(ssh2_disconnect_reasons)) ?
                     ssh2_disconnect_reasons[reason] : _T("unknown"),
                     buf+nowlen));
	    sfree(buf);
            crStop(0);
        }
...
static Bignum ssh2_pkt_getmp(Ssh ssh)
{
	...
	return NULL;
    if (p[0] & 0x80) {
	bombout((_T("internal error: Can't handle negative mpints")));
	return NULL;
    }
...
static void ssh_detect_bugs(Ssh ssh, char *vstring)
{
	...
	__logtevent((_T("We believe remote version has SSH1 ignore bug")));
    }
	...
	__logtevent(_T("We believe remote version needs a plain SSH1 password"));
    }
	...
	__logtevent(_T("We believe remote version can't handle RSA authentication"));
    }
	...
	__logtevent(_T("We believe remote version has SSH2 HMAC bug"));
    }
	...
	__logtevent(_T("We believe remote version has SSH2 key-derivation bug"));
    }
	...
	__logtevent(_T("We believe remote version has SSH2 RSA padding bug"));
    }
	...
	__logtevent(_T("We believe remote version has SSH2 public-key-session-ID bug"));
    }
	...
	__logtevent(_T("We believe remote version has SSH2 DH group exchange bug"));
    }
}
...
static int do_ssh_init(Ssh ssh, unsigned char c)
{
	...
	bombout((_T("SSH protocol version 1 required by user but not provided by server")));
	crStop(0);
    }
    if (ssh->cfg.sshprot == 3 && !s->proto2) {
	bombout((_T("SSH protocol version 2 required by user but not provided by server")));
	crStop(0);
    }
	...
	__logtevent(_T("Using SSH protocol version 2"));
	...
	__logtevent(_T("Using SSH protocol version 1"));
	sk_write(ssh->s, verstring, strlen(verstring));
...
static int ssh_closing(Plug plug, const TCHAR *error_msg, int error_code,
		       int calling_back)
{
    Ssh ssh = (Ssh) plug;
    ssh_do_close(ssh);
    if (error_msg) {
		...
		__logtevent(error_msg);
		connection_fatal(ssh->frontend, _T("%s"), error_msg);
    } else {
 ...
 * freed by the caller.
 */
static const TCHAR *connect_to_host(Ssh ssh, char *host, int port,
...
static int do_ssh1_login(Ssh ssh, unsigned char *in, int inlen, int ispkt)
{
	...
	bombout((_T("Public key packet not received")));
	crStop(0);
    }

    __logtevent(_T("Received public keys"));
    ...
    {
	char logmsg[80];
	__logtevent(_T("Host key fingerprint is:"));
	strcpy(logmsg, "      ");
    ...
    __logtevent(_T("Encrypted session key"));
	...
	for (i = 0; !cipher_chosen && i < CIPHER_MAX; i++) {
		...
		__logtevent(_T("AES not supported in SSH1, skipping"));
	    } else {
		switch (next_cipher) {
	...
	if (!cipher_chosen) {
		...
		bombout((_T("Server violates SSH 1 protocol by not ")
			 _T("supporting 3DES encryption")));
	    else
		/* shouldn't happen */
		bombout((_T("No supported ciphers found")));
	    crStop(0);
	}
	...
	__logtevent(_T("Using 3DES encryption"));
	break;
      case SSH_CIPHER_DES:
	__logtevent(_T("Using single-DES encryption"));
	break;
      case SSH_CIPHER_BLOWFISH:
	__logtevent(_T("Using Blowfish encryption"));
	break;
    }
    ...
    __logtevent(_T("Trying to enable encryption..."));
    ...
    __logtevent(_T("Installing CRC compensation attack detector"));

    crWaitUntil(ispkt);

    if (ssh->pktin.type != SSH1_SMSG_SUCCESS) {
	bombout((_T("Encryption not successfully enabled")));
	crStop(0);
    }

    __logtevent(_T("Successfully started encryption"));
	...
	if ((flags & FLAG_INTERACTIVE) && !*ssh->cfg.username) {
		...
		if (!ssh_get_line("login as: ",
		     ...
		     * Terminate.
		     */
		    __logtevent(_T("No username provided. Abandoning session."));
	...
	if (agent_exists() && !s->tried_agent) {
	    ...
	    s->authed = FALSE;
	    s->tried_agent = 1;
	    __logtevent(_T("Pageant is running. Requesting keys."));
		...
		do {
		    crReturn(0);
		    if (ispkt) {
			bombout((_T("Unexpected data from server while waiting")
				 _T(" for agent response")));
			crStop(0);
		    }
		} while (ispkt || inlen > 0);
		...
		for (s->keyi = 0; s->keyi < s->nkeys; s->keyi++) {
		    {
			TCHAR buf[64];
			_stprintf(buf, _T("Trying Pageant key #%d"), s->keyi);
			__logtevent(buf);
		    }
		    if (s->publickey_blob &&
			!memcmp(s->p, s->publickey_blob,
				s->publickey_bloblen)) {
			__logtevent(_T("This key matches configured key file"));
			s->tried_publickey = 1;
		    }
		    s->p += 4;
			...
			__logtevent(_T("Key refused"));
			continue;
		    }
		    __logtevent(_T("Received RSA challenge"));
			...
			*q++ = SSH1_AGENTC_RSA_CHALLENGE;
			...
			if (!agent_query(agentreq, len + 4, &vret, &retlen,
				...
				crReturn(0);
				if (ispkt) {
				    bombout((_T("Unexpected data from server")
					     _T(" while waiting for agent")
					     _T(" response")));
				    crStop(0);
				}
			    } while (ispkt || inlen > 0);
			...
			if (ret) {
				...
				__logtevent(_T("Sending Pageant's response"));
				...
				if (ssh->pktin.type == SSH1_SMSG_SUCCESS) {
				    __logtevent
					(_T("Pageant's response accepted"));
				    ...
				    __logtevent
					(_T("Pageant's response not accepted"));
			    } else {
				__logtevent
				    (_T("Pageant failed to answer challenge"));
				sfree(ret);
			    }
			} else {
			    __logtevent(_T("No reply received from Pageant"));
			}
		    }
	...
	if (ssh->cfg.try_tis_auth &&
		...
		__logtevent(_T("Received TIS challenge"));
	...
	if (ssh_get_line) {
		...
		send_packet(ssh, SSH1_MSG_DISCONNECT,
			    ...
			    PKT_END);
		__logtevent(_T("Unable to authenticate"));
		connection_fatal(ssh->frontend, _T("Unable to authenticate"));
                ssh_closing((Plug)ssh, NULL, 0, 0);
		crStop(1);
	    }
	} else {
	...
	if (s->pwpkt_type == SSH1_CMSG_AUTH_RSA) {
	    ...
	    if (ssh->pktin.type != SSH1_SMSG_AUTH_RSA_CHALLENGE) {
		bombout((_T("Bizarre response to offer of public key")));
		crStop(0);
	    }
		...
		bombout((_T("Bizarre response to RSA authentication response")));
		crStop(0);
	    }
		...
		if (!(ssh->remote_bugs & BUG_CHOKES_ON_SSH1_IGNORE)) {
		    ...
		    __logtevent(_T("Sending password with camouflage packets"));
		    ...
		    __logtevent(_T("Sending length-padded password"));
		    ...
		    int len;
		    len = strlen(s->password);
		    __logtevent(_T("Sending unpadded password"));
		    send_packet(ssh, s->pwpkt_type, PKT_INT, len,
	...
	__logtevent(_T("Sent password"));
	memset(s->password, 0, strlen(s->password));
	crWaitUntil(ispkt);
	if (ssh->pktin.type == SSH1_SMSG_FAILURE) {
	    if (flags & FLAG_VERBOSE)
		c_write_str(ssh, "Access denied\r\n");
	    __logtevent(_T("Authentication refused"));
	} else if (ssh->pktin.type != SSH1_SMSG_SUCCESS) {
	    bombout((_T("Strange packet received, type %d"), ssh->pktin.type));
	    crStop(0);
	}
    }

    __logtevent(_T("Authentication successful"));
...
void sshfwd_close(struct ssh_channel *c)
{
	...
	if (c->type == CHAN_X11) {
	    c->u.x11.s = NULL;
	    __logtevent(_T("Forwarded X11 connection terminated"));
	} else if (c->type == CHAN_SOCKDATA ||
		   c->type == CHAN_SOCKDATA_DORMANT) {
	    c->u.pfd.s = NULL;
	    __logtevent(_T("Forwarded port closed"));
	}
    }
}
...
static void ssh1_protocol(Ssh ssh, unsigned char *in, int inlen, int ispkt)
{
	...
	__logtevent(_T("Requesting agent forwarding"));
	...
	if (ssh->pktin.type != SSH1_SMSG_SUCCESS
	    ...
	    bombout((_T("Protocol confusion")));
	    crStopV;
	} else if (ssh->pktin.type == SSH1_SMSG_FAILURE) {
	    __logtevent(_T("Agent forwarding refused"));
	} else {
	    __logtevent(_T("Agent forwarding enabled"));
	...
	__logtevent(_T("Requesting X11 forwarding"));
	...
	if (ssh->pktin.type != SSH1_SMSG_SUCCESS
	    ...
	    __logtevent(_T("X11 forwarding refused"));
	} else {
	    __logtevent(_T("X11 forwarding enabled"));
	...
	while (*ssh->portfwd_strptr) {
		...
		if (type == 'L') {
			...
			__logtevent(_T("Remote port forwarding enabled"));
		    }
		}
	    }
	}
	...
	__logtevent(_T("Started compression"));
	ssh->v1_compressing = TRUE;
	ssh->cs_comp_ctx = zlib_compress_init();
	__logtevent(_T("Initialised zlib (RFC1950) compression"));
	ssh->sc_comp_ctx = zlib_decompress_init();
	__logtevent(_T("Initialised zlib (RFC1950) decompression"));
    }
	...
	__logtevent(_T("Started session"));
    }
	...
	if (ispkt) {
		...
		if (!ssh->v1_stdout_throttling && bufsize > SSH1_BUFFER_LIMIT) {
		    ...
		    ssh1_throttle(ssh, +1);
                ...
                ssh_closing((Plug)ssh, NULL, 0, 0);
		__logtevent(_T("Received disconnect request"));
		crStopV;
		...
		__logtevent(_T("Received X11 connect request"));
		/* Refuse if X11 forwarding is disabled. */
		if (!ssh->X11_fwd_enabled) {
		    send_packet(ssh, SSH1_MSG_CHANNEL_OPEN_FAILURE,
				PKT_INT, GET_32BIT(ssh->pktin.body), PKT_END);
		    __logtevent(_T("Rejected X11 connect request"));
		} else {
			...
			__logtevent(_T("opening X11 forward connection failed"));
			sfree(c);
			send_packet(ssh, SSH1_MSG_CHANNEL_OPEN_FAILURE,
			...
			__logtevent
			    (_T("opening X11 forward connection succeeded"));
			...
			send_packet(ssh, SSH1_MSG_CHANNEL_OPEN_CONFIRMATION,
				    ...
				    c->localid, PKT_END);
			__logtevent(_T("Opened X11 forward channel"));
		    }
		}
		...
		*h = 0;
		...
		if (find234(ssh->rportfwds, &pf, NULL) == NULL) {
		    ...
		    if (e != NULL) {
			TCHAR buf[256];
			_stprintf(buf, _T("Port open failed: %s"), e);
			__logtevent(buf);
			sfree(c);
			send_packet(ssh, SSH1_MSG_CHANNEL_OPEN_FAILURE,
				    PKT_INT, GET_32BIT(ssh->pktin.body),
			...
			send_packet(ssh, SSH1_MSG_CHANNEL_OPEN_CONFIRMATION,
				    PKT_INT, c->remoteid, PKT_INT,
				    c->localid, PKT_END);
			__logtevent(_T("Forwarded port opened successfully"));
		    }
		}
		...
		c = find234(ssh->channels, &remoteid, ssh_channelfind);
		if (c && c->type == CHAN_SOCKDATA_DORMANT) {
		    __logtevent(_T("Forwarded connection refused by server"));
		    pfd_close(c->u.pfd.s);
		    del234(ssh->channels, c);
		    sfree(c);
		}
		...
		if (c && ((int)c->remoteid) != -1) {
		    ...
		    bombout((_T("Received CHANNEL_CLOSE%s for %s channel %d\n"),
			     ssh->pktin.type == SSH1_MSG_CHANNEL_CLOSE ? _T("") :
			     _T("_CONFIRMATION"), c ? _T("half-open") : _T("nonexistent"),
			     i));
		    crStopV;
		}
		...
		_stprintf(buf, _T("Server sent command exit status %d"),
			ssh->exitcode);
		__logtevent(buf);
...
static int do_ssh2_transport(Ssh ssh, unsigned char *in, int inlen, int ispkt)
{
  ...
  begin_key_exchange:
	...
	if (ssh->pktin.type != SSH2_MSG_KEXINIT) {
	    bombout((_T("expected key exchange packet from server")));
	    crStop(0);
	}
	ssh->kex = NULL;
	ssh->hostkey = NULL;
	...
	if (!s->cscipher_tobe) {
#ifdef _UNICODE
	    wchar_t *wstr = (str ? ansi2unicode(str) : NULL);
	    bombout((L"Couldn't agree a client-to-server cipher (available: %s)",
		     wstr ? wstr : L"(null)"));
	    sfree(wstr);
#else
...
#ifdef _UNICODE
	    wchar_t *wstr = (str ? ansi2unicode(str) : NULL);
	    bombout((L"Couldn't agree a server-to-client cipher (available: %s)",
		     wstr ? wstr : L"(null)"));
	    sfree(wstr);
#else
	...
	__logtevent(_T("Doing Diffie-Hellman group exchange"));
	...
	if (ssh->pktin.type != SSH2_MSG_KEX_DH_GEX_GROUP) {
	    bombout((_T("expected key exchange group packet from server")));
	    crStop(0);
	}
    ...
    __logtevent(_T("Doing Diffie-Hellman key exchange"));
    /*
	...
	bombout((_T("expected key exchange reply packet from server")));
	crStop(0);
    }
	...
	bombout((_T("Server's host key did not match the signature supplied")));
	crStop(0);
    }
	...
	bombout((_T("expected new-keys packet from server")));
	crStop(0);
    }
    ...
    __logtevent(_T("Server initiated key re-exchange"));
...
static void do_ssh2_authconn(Ssh ssh, unsigned char *in, int inlen, int ispkt)
{
	...
	bombout((_T("Server refused user authentication protocol")));
	crStopV;
    }
	...
	while (1) {
		...
		bombout((_T("Strange packet received during authentication: type %d"),
			 ssh->pktin.type));
		crStopV;
	    }
		...
		if (!ssh2_pkt_getbool(ssh)) {
		    ...
		    __logtevent(_T("Further authentication required"));
		}
		...
		if (!agent_query(s->request, 5, &r, &s->responselen,
			...
			crReturnV;
			if (ispkt) {
			    bombout((_T("Unexpected data from server while")
				     _T(" waiting for agent response")));
			    crStopV;
			}
		    } while (ispkt || inlen > 0);
		    r = ssh->agent_response;
		...
		if (s->response && s->responselen >= 5 &&
		    ...
		    {
			TCHAR buf[64];
			_stprintf(buf, _T("Pageant has %d SSH2 keys"), s->nkeys);
			__logtevent(buf);
		    }
		    for (s->keyi = 0; s->keyi < s->nkeys; s->keyi++) {
			void *vret;
		...
		if (pub_blob) {
		    ...
		    ssh2_pkt_addstring_data(ssh, (char *)pub_blob,
					    pub_blob_len);
		    ssh2_pkt_send(ssh);
		    __logtevent(_T("Offered public key"));	/* FIXME */
		    ...
		    __logtevent(_T("Offer of public key accepted"));
		    /*
		...
		if (ssh->pktin.type != SSH2_MSG_USERAUTH_INFO_REQUEST) {
		    if (ssh->pktin.type == SSH2_MSG_USERAUTH_FAILURE)
			s->gotit = TRUE;
		    __logtevent(_T("Keyboard-interactive authentication refused"));
		...
		c_write_str(ssh, "No supported authentication methods"
			    " left to try!\r\n");
		__logtevent(_T("No supported authentications offered.")
			 _T(" Disconnecting"));
		ssh2_pkt_init(ssh, SSH2_MSG_DISCONNECT);
		ssh2_pkt_adduint32(ssh, SSH2_DISCONNECT_BY_APPLICATION);
	...
	bombout((_T("Server refused to open a session")));
	crStopV;
	/* FIXME: error data comes back in FAILURE packet */
    }
    if (ssh2_pkt_getuint32(ssh) != ssh->mainchan->localid) {
	bombout((_T("Server's channel confirmation cited wrong channel")));
	crStopV;
    }
    ...
    __logtevent(_T("Opened channel for session"));
	...
	if (ssh->pktin.type != SSH2_MSG_CHANNEL_SUCCESS) {
		...
		bombout((_T("Unexpected response to X11 forwarding request:")
			 _T(" packet type %d"), ssh->pktin.type));
		crStopV;
	    }
	    __logtevent(_T("X11 forwarding refused"));
	} else {
	...
	while (*ssh->portfwd_strptr) {
		...
		if (type == 'L') {
			...
			if (ssh->pktin.type != SSH2_MSG_REQUEST_SUCCESS) {
				...
				bombout((_T("Unexpected response to port ")
					 _T("forwarding request: packet type %d"),
					 ssh->pktin.type));
				crStopV;
			    }
			    __logtevent(_T("Server refused this port forwarding"));
			} else {
			    __logtevent(_T("Remote port forwarding enabled"));
			}
		    }
     ...
     */
    if (ssh->cfg.agentfwd && agent_exists()) {
	__logtevent(_T("Requesting OpenSSH-style agent forwarding"));
	ssh2_pkt_init(ssh, SSH2_MSG_CHANNEL_REQUEST);
	ssh2_pkt_adduint32(ssh, ssh->mainchan->remoteid);
	...
	if (ssh->pktin.type != SSH2_MSG_CHANNEL_SUCCESS) {
	    if (ssh->pktin.type != SSH2_MSG_CHANNEL_FAILURE) {
		bombout((_T("Unexpected response to agent forwarding request:")
			 _T(" packet type %d"), ssh->pktin.type));
		crStopV;
	    }
	    __logtevent(_T("Agent forwarding refused"));
	...
	if (ssh->pktin.type != SSH2_MSG_CHANNEL_SUCCESS) {
	    if (ssh->pktin.type != SSH2_MSG_CHANNEL_FAILURE) {
		bombout((_T("Unexpected response to pty request:")
			 _T(" packet type %d"), ssh->pktin.type));
		crStopV;
	    }
	    c_write_str(ssh, "Server refused to allocate pty\r\n");
	...
	if (ssh->pktin.type != SSH2_MSG_CHANNEL_SUCCESS) {
	    if (ssh->pktin.type != SSH2_MSG_CHANNEL_FAILURE) {
		bombout((_T("Unexpected response to shell/command request:")
			 _T(" packet type %d"), ssh->pktin.type));
		crStopV;
	    }
	    /*
		...
		__logtevent(_T("Primary command failed; attempting fallback"));
		ssh->fallback_cmd = TRUE;
		continue;
	    }
	    bombout((_T("Server refused to start a shell/command")));
	    crStopV;
	} else {
	    __logtevent(_T("Started a shell/command"));
	}
	break;
    }
	...
	if (ispkt) {
		...
		c = find234(ssh->channels, &i, ssh_channelfind);
		if (!c || ((int)c->remoteid) == -1) {
		    bombout((_T("Received CHANNEL_CLOSE for %s channel %d\n"),
			     c ? _T("half-open") : _T("nonexistent"), i));
		    crStopV;
		}
		...
		if (count234(ssh->channels) == 0) {
		    __logtevent(_T("All channels closed. Disconnecting"));
#if 0
                    /*
...
#ifdef _UNICODE
		    {
			wchar_t *wbuf = ansi2unicode(buf);
			connection_fatal(ssh->frontend, _T("%s"), wbuf);
			sfree(wbuf);
		    }
#else
		...
		if (typelen == 3 && !memcmp(type, "x11", 3)) {
			...
			const TCHAR *e = pfd_newconnect(&c->u.pfd.s,
						       realpf->dhost,
						       realpf->dport, c,
						       &ssh->cfg);
			...
			if (e != NULL) {
			    _logteventf(ssh, _T("Port open failed: %s"), e);
 ...
 * Returns an error message, or NULL on success.
 */
static const TCHAR *ssh_init(void *frontend_handle, void **backend_handle,
			    Config *cfg,
...
static void ssh_special(void *handle, Telnet_Special code)
{
	...
	__logtevent(_T("Sent EOF message"));
	...
	if (ssh->state == SSH_STATE_CLOSED
	    || ssh->state == SSH_STATE_PREPACKET) return;
	if (ssh->version == 1) {
	    __logtevent(_T("Unable to send BREAK signal in SSH1"));
	} else {
	    ssh2_pkt_init(ssh, SSH2_MSG_CHANNEL_REQUEST);
	    ssh2_pkt_adduint32(ssh, ssh->mainchan->remoteid);
