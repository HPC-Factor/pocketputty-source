...
static int raw_closing(Plug plug, const TCHAR *error_msg, int error_code,
		       int calling_back)
{
    Raw raw = (Raw) plug;
	...
	_logtevent(raw->frontend, error_msg);
	connection_fatal(raw->frontend, _T("%s"), error_msg);
 ...
 * freed by the caller.
 */
static const TCHAR *raw_init(void *frontend_handle, void **backend_handle,
			    Config *cfg,
