...
static const TCHAR * sk_proxy_socket_error (Socket s)
{
    Proxy_Socket ps = (Proxy_Socket) s;
...
static int plug_proxy_closing (Plug p, const TCHAR *error_msg,
		      ...
		      const Config *cfg)
{
	...
	if (cfg->proxy_type == PROXY_HTTP) {
	    ...
	    ret->error = _T("Proxy error: Unknown proxy method");
	    return (Socket) ret;
	}
	...
	if (sk_addr_error(proxy_addr) != NULL) {
	    ret->error = _T("Proxy error: Unable to resolve proxy host name");
	    return (Socket)ret;
	}
...
int proxy_http_negotiate (Proxy_Socket p, int change)
{
	...
	if (p->state == 1) {
		...
		plug_closing(p->plug, _T("Proxy error: HTTP response was absent"),
...
#ifdef _UNICODE
			data_status = ansi2unicode(data + status);
			buf = _duptprintf(_T("Proxy error: %s"), data_status);
			sfree(data_status);
    ...
    plug_closing(p->plug, _T("Proxy error: unexpected proxy error"),
...
int proxy_socks4_negotiate (Proxy_Socket p, int change)
{
	...
	if (type == ADDRTYPE_IPV6) {
	    plug_closing(p->plug, _T("Proxy error: SOCKS version 4 does")
			 _T(" not support IPv6"), PROXY_ERROR_GENERAL, 0);
	    return 1;
	...
	if (p->state == 1) {
	    ...
	    if (data[0] != 0) {
		plug_closing(p->plug, _T("Proxy error: SOCKS proxy responded with ")
				      _T("unexpected reply code version"),
			     PROXY_ERROR_GENERAL, 0);
		return 1;
	    }
		...
		switch (data[1]) {
		  case 92:
		    plug_closing(p->plug, _T("Proxy error: SOCKS server wanted IDENTD on client"),
				 PROXY_ERROR_GENERAL, 0);
		    break;
		  case 93:
		    plug_closing(p->plug, _T("Proxy error: Username and IDENTD on client don't agree"),
				 PROXY_ERROR_GENERAL, 0);
		    break;
		  case 91:
		  default:
		    plug_closing(p->plug, _T("Proxy error: Error while communicating with proxy"),
				 PROXY_ERROR_GENERAL, 0);
		    break;
		}
...
int proxy_socks5_negotiate (Proxy_Socket p, int change)
{
	...
	if (p->state == 1) {
	    ...
	    if (data[0] != 5) {
		plug_closing(p->plug, _T("Proxy error: SOCKS proxy returned unexpected version"),
			     PROXY_ERROR_GENERAL, 0);
		return 1;
	    }
		...
		plug_closing(p->plug, _T("Proxy error: SOCKS proxy did not accept our authentication"),
			     PROXY_ERROR_GENERAL, 0);
		return 1;
	    }
	    bufchain_consume(&p->pending_input_data, 2);
	...
	if (p->state == 7) {
	    ...
	    if (data[0] != 1) {
		plug_closing(p->plug, _T("Proxy error: SOCKS password ")
			     _T("subnegotiation contained wrong version number"),
			     PROXY_ERROR_GENERAL, 0);
		return 1;
	    }
		...
		plug_closing(p->plug, _T("Proxy error: SOCKS proxy refused")
			     _T(" password authentication"),
			     PROXY_ERROR_GENERAL, 0);
		return 1;
	    }
	...
	if (p->state == 3) {
	    ...
	    if (data[0] != 5) {
		plug_closing(p->plug, _T("Proxy error: SOCKS proxy returned wrong version number"),
			     PROXY_ERROR_GENERAL, 0);
		return 1;
	    }
	    ...
	    if (data[1] != 0) {
			...
			_tcscpy(buf, _T("Proxy error: "));

			switch (data[1]) {
			  case 1: _tcscat(buf, _T("General SOCKS server failure")); break;
			  case 2: _tcscat(buf, _T("Connection not allowed by ruleset")); break;
			  case 3: _tcscat(buf, _T("Network unreachable")); break;
			  case 4: _tcscat(buf, _T("Host unreachable")); break;
			  case 5: _tcscat(buf, _T("Connection refused")); break;
			  case 6: _tcscat(buf, _T("TTL expired")); break;
			  case 7: _tcscat(buf, _T("Command not supported")); break;
			  case 8: _tcscat(buf, _T("Address type not supported")); break;
			  default: _stprintf(buf+_tcslen(buf),
					   _T("Unrecognised SOCKS error code %d"),
					   data[1]);
				break;
		}
		plug_closing(p->plug, buf, PROXY_ERROR_GENERAL, 0);
	    ...
	    switch (data[3]) {
	      ...
	      default:
		plug_closing(p->plug, _T("Proxy error: SOCKS proxy returned ")
			     _T("unrecognised address format"),
			     PROXY_ERROR_GENERAL, 0);
		return 1;
	    }
	...
	if (p->state == 4) {
	    ...
	    plug_closing(p->plug, _T("Proxy error: We don't support GSSAPI authentication"),
			 PROXY_ERROR_GENERAL, 0);
	    return 1;
	}
	...
	if (p->state == 5) {
		...
		plug_closing(p->plug, _T("Proxy error: Server chose ")
			     _T("username/password authentication but we ")
			     _T("didn't offer it!"),
			 PROXY_ERROR_GENERAL, 0);
	    return 1;
	}
	...
	if (p->state == 6) {
	    ...
	    plug_closing(p->plug, _T("Proxy error: We don't support CHAP authentication"),
			 PROXY_ERROR_GENERAL, 0);
	    return 1;
	}
