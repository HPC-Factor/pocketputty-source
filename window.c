...
#ifdef _WIN32_WCE
static HWND hwndCB;
static HTHREAD g_hWSAEmulationThread = 0;
static BOOL g_bTerminate; // Thread termination flag
...
static TCHAR *window_name, *icon_name;
...
#ifdef _WIN32_WCE
//Aleq
DWORD WINAPI WSAAsyncEmulationThreadProc(LPVOID lpParameter) {
    WSAAsyncEmulationParam *wsaAEP = (WSAAsyncEmulationParam *) lpParameter;
    u_long argp;
    g_bTerminate = FALSE;

    while (!g_bTerminate) {
	if (1 /* wsaAEP->socket */) {
	    int returnval = ioctlsocket(wsaAEP->socket, FIONREAD, &argp);
	    if (returnval == SOCKET_ERROR) {
		    fatalbox(_T("ioctlsocket(): SOCKET_ERROR"));
		    Sleep(5000);
	    } else if (argp > 0) {
		SendMessage(hwnd, wsaAEP->Msg, (WPARAM) wsaAEP->socket, FD_READ);
	    }
	} else {
	    // socket is null
	    fatalbox((_T("Null socket")));
	    debug(((_T("NULL SOCKET IN THREAD"))));
	    Sleep(500);
	}
	// Small idle
	Sleep(1);
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPTSTR cmdline, int show)
{
    WNDCLASS wndclass;
    MSG msg;
    ...
    if (!init_winver())
    {
	TCHAR *str = _duptprintf(_T("%s Fatal Error"), appname);
	MessageBox(NULL, _T("Windows refuses to report a version"),
     ...
     */
    {
        TCHAR b[2048], *p, *q, *r;
        FILE *fp;
        GetModuleFileName(NULL, b, sizeof(b) / sizeof(TCHAR) - 1);
        r = b;
        p = _tcsrchr(b, _T('\\'));
        if (p && p >= r) r = p+1;
        q = _tcsrchr(b, _T(':'));
        if (q && q >= r) r = q+1;
        _tcscpy(r, _T("putty.hlp"));
        if ( (fp = _tfopen(b, _T("r"))) != NULL) {
            help_path = _duptcs(b);
            fclose(fp);
        } else
            help_path = NULL;
        _tcscpy(r, _T("putty.cnt"));
        if ( (fp = _tfopen(b, _T("r"))) != NULL) {
	...
	if (*p == _T('@')) {
	    ...
	    HANDLE filemap;
	    Config *cp;
	    if (_stscanf(p + 1, _T("%p"), &filemap) == 1 &&
		...
		ret = cmdline_process_param(p, i+1<argc?argv[i+1]:NULL,
					    1, &cfg);
		if (ret == -2) {
		    cmdline_error(_T("option \"%s\" requires an argument"), p);
		} else if (ret == 2) {
		    i++;	       /* skip next argument */
		} else if (ret == 1) {
		    continue;	       /* nothing further needs doing */
		} else if (!_tcscmp(p, _T("-cleanup"))) {
		    /*
		     ...
		     */
		    TCHAR *s1, *s2;
		    s1 = _duptprintf(_T("This procedure will remove ALL Registry\n")
				   _T("entries associated with %s, and will\n")
				   _T("also remove the random seed file.\n")
				   _T("\n")
				   _T("THIS PROCESS WILL DESTROY YOUR SAVED\n")
				   _T("SESSIONS. Are you really sure you want\n")
				   _T("to continue?"), appname);
		    s2 = _duptprintf(_T("%s Warning"), appname);
			 ...
			 */
			int ret = cmdline_process_param(_T("-P"), p, 1, &cfg);
			assert(ret == 2);
		    } else if (!_tcsncmp(q, _T("telnet:"), 7)) {
			/*
			...
			if (c == _T(':'))
			    ...
			    cfg.port = -1;
#ifdef _UNICODE
			wcstombs(cfg.host, q, sizeof(cfg.host) - 1);
#else
		    ...
		    cmdline_error(_T("unknown option \"%s\""), p);
		}
	    }
	}
...
//	cfg.scrollbar_in_fullscreen = 1;
	...
	if (back == NULL) {
	    TCHAR *str = _duptprintf(_T("%s Internal Error"), appname);
	    MessageBox(NULL, _T("Unsupported protocol number found"),
		       str, MB_OK | MB_ICONEXCLAMATION);
	    sfree(str);
	    cleanup_exit(1);
	}
    ...
    if (cfg.port == 0) {
	TCHAR *str = _duptprintf(_T("%s Internal Error"), appname);
	MessageBox(NULL, _T("Invalid Port Number"),
		   str, MB_OK | MB_ICONEXCLAMATION);
	sfree(str);
	cleanup_exit(1);
    }
...
//    }
    {
	hwnd = CreateWindow(appname, appname, WS_VISIBLE,
		CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, 
		NULL, NULL, inst, NULL);

	// On WinCE caret flickers awfully
	HideCaret(hwnd);
     ...
     */
#ifdef _WIN32_WCE
    // Shrink window so the bottom bar is visible
    if (hwnd && hwndCB) {
	    RECT rc;
	    RECT rcMenuBar;
...
debug((_T("Starting the connection\n")));
    /*
     * Start up the telnet connection.
     */
    {
	const TCHAR *error;
	TCHAR msg[1024], *title;
	char *realhost;
	...
	if (error) {
	    TCHAR *str = _duptprintf(_T("%s Error"), appname);
	    _stprintf(msg, _T("Unable to open connection to\n")
		    _T("%.800s\n") _T("%s"), cfg.host, error);
...
#ifdef _UNICODE
	    wchar_t *wrealhost = ansi2unicode(realhost);
	    _stprintf(msg, _T("%s - %s"), wrealhost, appname);
	    sfree(wrealhost);
#else
	    _stprintf(msg, _T("%s - %s"), realhost, appname);
#endif;
	    title = msg;
	}
	sfree(realhost);
	...
	AppendMenu(m, MF_ENABLED, IDM_SHOWLOG, _T("&Event Log"));
	AppendMenu(m, MF_SEPARATOR, 0, 0);
	AppendMenu(m, MF_ENABLED, IDM_NEWSESS, _T("Ne&w Session..."));
	AppendMenu(m, MF_ENABLED, IDM_DUPSESS, _T("&Duplicate Session"));
	s = CreateMenu();
	...
	for (i = 1;
	    ...
	    AppendMenu(s, MF_ENABLED, IDM_SAVED_MIN + (16 * i),
		       sesslist.sessions[i]);
	AppendMenu(m, MF_POPUP | MF_ENABLED, (UINT) s, _T("Sa&ved Sessions"));
	AppendMenu(m, MF_ENABLED, IDM_RECONF, _T("Chan&ge Settings..."));
	AppendMenu(m, MF_SEPARATOR, 0, 0);
	AppendMenu(m, MF_ENABLED, IDM_COPYALL, _T("C&opy All to Clipboard"));
	AppendMenu(m, MF_ENABLED, IDM_CLRSB, _T("C&lear Scrollback"));
	AppendMenu(m, MF_ENABLED, IDM_RESET, _T("Rese&t Terminal"));
	AppendMenu(m, MF_SEPARATOR, 0, 0);
	AppendMenu(m, (cfg.resize_action == RESIZE_DISABLED) ?
		   MF_GRAYED : MF_ENABLED, IDM_FULLSCREEN, _T("&Full Screen"));
	AppendMenu(m, MF_SEPARATOR, 0, 0);
        if (help_path)
            AppendMenu(m, MF_ENABLED, IDM_HELP, _T("&Help"));
	str = dupprintf(_T("&About %s"), appname);
...
debug((_T("GetForegroundWindow(),")));
    term->has_focus = (GetForegroundWindow() == hwnd);
debug((_T("UpdateWindow(),")));
    UpdateWindow(hwnd);


debug((_T("Message queue processing:\n\n")));
...
debug((_T("Message queue processing FINISHED\n")));
...
#ifdef _WIN32_WCE
void ShutdownWSAEmulationThread() {
    if (g_hWSAEmulationThread) {
    g_bTerminate = TRUE;
    Sleep(1);
    if (WaitForSingleObject(g_hWSAEmulationThread, 4000) == WAIT_TIMEOUT)
	TerminateThread(g_hWSAEmulationThread, 1);
	CloseHandle(g_hWSAEmulationThread);
	g_hWSAEmulationThread = 0;
	session_closed = TRUE;
    }
}
#endif
 ...
 */
TCHAR *do_select(SOCKET skt, int startup)
{
#ifndef _WIN32_WCE
    int msg, events;
    if (startup) {
    ...
    if (!hwnd)
	return _T("do_select(): internal error (hwnd==NULL)");
    if (p_WSAAsyncSelect(skt, hwnd, msg, events) == SOCKET_ERROR) {
	switch (p_WSAGetLastError()) {
	  case WSAENETDOWN:
	    return _T("Network is down");
	  default:
	    return _T("WSAAsyncSelect(): unknown error");
	}
    }
#else
    // Windows CE:
	...
	// Startup
	DWORD dwThreadId;
	WSAAsyncEmulationParam *wsaAEP = snew(WSAAsyncEmulationParam); // Thread is responsible for deallocation

	wsaAEP->Msg = WM_NETEVENT;
	wsaAEP->socket = skt;

	if (g_hWSAEmulationThread)
	    CloseHandle(g_hWSAEmulationThread);
	g_hWSAEmulationThread  = CreateThread(NULL, 0, WSAAsyncEmulationThreadProc, wsaAEP, 0, &dwThreadId);
	if (g_hWSAEmulationThread  == NULL) 
	    return (_T("Could not crate socket abstraction thread"));
//	else {
//	    CloseHandle(g_hWSAEmulationThread);
//	}
    } else {
	ShutdownWSAEmulationThread();
    }
#endif
    return NULL;
}
 ...
 */
void connection_fatal(void *frontend, TCHAR *fmt, ...)
{
    va_list ap;
    TCHAR *stuff, morestuff[100];

    va_start(ap, fmt);
    stuff = _dupvtprintf(fmt, ap);
    va_end(ap);
    _stprintf(morestuff, _T("%.70s Fatal Error"), appname);
	...
	session_closed = TRUE;
	_stprintf(morestuff, _T("%.70s (inactive)"), appname);
...
void cmdline_error(TCHAR *fmt, ...)
{
    ...
    stuff = _dupvtprintf(fmt, ap);
    va_end(ap);
    _stprintf(morestuff, _T("%.70s Command Line Error"), appname);
    MessageBox(hwnd, stuff, morestuff, MB_ICONERROR | MB_OK);
    sfree(stuff);
    exit(1);
}
...
static void enact_pending_netevent(void)
{
	...
	if (cfg.close_on_exit == FORCE_ON ||
	    ...
	    set_icon(NULL, morestuff);
	    set_title(NULL, morestuff);
	    MessageBox(hwnd, _T("Connection closed by remote host"),
...
#define f(i,c,w,u) \
    ...
    _tcsnccpy(lfont.lfFaceName, cfg.font.name, LF_FACESIZE);
    ...
    RDB_debug(23, (_T("Primary font H=%d, AW=%d, MW=%d"),
	...
	ExtTextOut(und_dc, 0, 0, ETO_OPAQUE, NULL, _T(" "), 1, NULL);
	gotit = FALSE;
...
static void reset_window(int reinit) {
    ...
    if (reinit>1) {
#ifdef RDB_DEBUG_PATCH
	RDB_debug(27, (_T("reset_window() -- Forced deinit")));
#endif
	deinit_fonts();
	init_fonts(0,0);
    }
	...
	InvalidateRect(hwnd, NULL, TRUE);
#ifdef RDB_DEBUG_PATCH
	RDB_debug(27, (_T("reset_window() -> Reposition terminal")));
#endif
    }
	...
	if (cfg.resize_action != RESIZE_TERM) {
		...
		InvalidateRect(hwnd, NULL, TRUE);
#ifdef RDB_DEBUG_PATCH
		RDB_debug(25, (_T("reset_window() -> Z font resize to (%d, %d)"),
		...
		InvalidateRect(hwnd, NULL, TRUE);
#ifdef RDB_DEBUG_PATCH
		RDB_debug(27, (_T("reset_window() -> Zoomed term_size")));
#endif
	    }
	}
	return;
    ...
    if (reinit>0) {
#ifdef RDB_DEBUG_PATCH
	RDB_debug(27, (_T("reset_window() -> Forced re-init")));
#endif
	...
	if (win_width != font_width*term->cols + offset_width*2 ||
...
#ifdef RDB_DEBUG_PATCH
		    RDB_debug(27,(_T("reset_window() -> term resize to (%d,%d)"),
			       height, width));
#endif
		}
	    }
	    ...
	    InvalidateRect(hwnd, NULL, TRUE);
#ifdef RDB_DEBUG_PATCH
	    RDB_debug(27, (_T("reset_window() -> window resize to (%d,%d)"),
			font_width*term->cols + extra_width,
			font_height*term->rows + extra_height));
#endif
	}
	...
	InvalidateRect(hwnd, NULL, TRUE);
#ifdef RDB_DEBUG_PATCH
	RDB_debug(25, (_T("reset_window() -> font resize to (%d,%d)"), 
		   font_width, font_height));
#endif
    }
}
...
static void set_input_locale(HKL kl)
{
    ...
    GetLocaleInfo(LOWORD(kl), LOCALE_IDEFAULTANSICODEPAGE,
		  lbuf, sizeof(lbuf) / sizeof(TCHAR));

    kbd_codepage = _ttoi(lbuf);
}
...
#ifdef DEBUG
debug_wprintf(_T("\nMessage %d(%x, %x): "), message, wParam, lParam);
#endif
    switch (message) {
#ifdef _WIN32_WCE
/*    case WM_SETTINGCHANGE:
	//fatalbox(_T("WM_SETTINGCHANGE"));
...
debug((_T("WM_TIMER: ")));
	...
	if (cfg.ping_interval > 0) {
	    ...
	    time(&now);
#endif
	    if (now - last_movement > (time_t) cfg.ping_interval) {
...
debug((_T("WM_CLOSE,")));
	{
	    TCHAR *str;
	    show_mouseptr(1);
	    str = _duptprintf(_T("%s Exit Confirmation"), appname);
	    if (!cfg.warn_on_close || session_closed ||
		MessageBox(hwnd,
			   _T("Are you sure you want to close this session?"),
...
debug((_T("IDM_RECONF,")));
		...
		GetWindowText(hwnd, cfg.wintitle, sizeof(cfg.wintitle) / sizeof(TCHAR));
		prev_cfg = cfg;
		...
		if (_tcscmp(cfg.font.name, prev_cfg.font.name) != 0 ||
...
debug((_T("SC_KEYMENU,")));
	    ...
	    if( lParam == 0 )
		PostMessage(hwnd, WM_CHAR, _T(' '), 0);
	    break;
	  case IDM_FULLSCREEN:
debug((_T("IDM_FULLSCREEN,")));
	    flip_full_screen();
	    break;
	  default:
	    if (wParam >= IDM_SAVED_MIN && wParam <= IDM_SAVED_MAX) {
debug((_T("IDM_SAVED_MIN..IDM_SAVED_MAX,")));
		SendMessage(hwnd, WM_SYSCOMMAND, IDM_SAVEDSESS, wParam);
	    }
	    if (wParam >= IDM_SPECIAL_MIN && wParam <= IDM_SPECIAL_MAX) {
		int i = (wParam - IDM_SPECIAL_MIN) / 0x10;
		int j;
debug((_T("IDM_SPECIAL_MIN..IDM_SPECIAL_MAX,")));
		/*
...
#endif
      case WM_IGNORE_CLIP:
debug((_T("WM_IGNORE_CLIP,")));
	ignore_clip = wParam;	       /* don't panic on DESTROYCLIPBOARD */
	break;
      case WM_DESTROYCLIPBOARD:
debug((_T("WM_DESTROYCLIPBOARD,")));
	if (!ignore_clip)
	    term_deselect(term);
	ignore_clip = FALSE;
	return 0;
...
#ifdef _WIN32_WCE
	last_movement = GetTickCount();
#else
	time(&last_movement);
#endif
	return 0;
      case WM_SETFOCUS:
debug((_T("WM_SETFOCUS,")));
	term->has_focus = TRUE;
	CreateCaret(hwnd, caretbm, font_width, font_height);
	ShowCaret(hwnd);
	flash_window(0);	       /* stop */
...
debug((_T("WM_MOVE,")));
	...
	break;
      case WM_SIZE:
#ifdef RDB_DEBUG_PATCH
	RDB_debug(27, (_T("WM_SIZE %s (%d,%d)"),
		(wParam == SIZE_MINIMIZED) ? _T("SIZE_MINIMIZED"):
		(wParam == SIZE_MAXIMIZED) ? _T("SIZE_MAXIMIZED"):
		(wParam == SIZE_RESTORED && resizing) ? _T("to"):
		(wParam == SIZE_RESTORED) ? _T("SIZE_RESTORED"):
		_T("..."),
...
//Aleq: WinCE calls this before having window_name with connection due to 
	    if (window_name)
...
debug((_T("do_text();")));
...
#ifdef _UNICODE
	TCHAR *wtext = ansi2unicode(text);
	ExtTextOut(hdc, x,
		   y - font_height * (lattr == LATTR_BOT) + text_adjust,
		   ETO_CLIPPED | ETO_OPAQUE, &line_box, wtext, len, IpDx);
#else
	ExtTextOut(hdc, x,
		   y - font_height * (lattr == LATTR_BOT) + text_adjust,
	...
	if (bold_mode == BOLD_SHADOW && (attr & ATTR_BOLD)) {
	    ...
	    ExtTextOut(hdc, x - 1,
		       y - font_height * (lattr ==
					  LATTR_BOT) + text_adjust,
		       ETO_CLIPPED, &line_box, wtext, len, IpDx);
#else
	    ExtTextOut(hdc, x - 1,
		       y - font_height * (lattr ==
					  LATTR_BOT) + text_adjust,
...
#if (UNDER_CE > 0 && UNDER_CE < 400)
	{
	    ...
	    poly[0].x = x;
	    poly[0].y = y + dec;
	    poly[1].x = x + len * char_width;
	    poly[1].y = y + dec;
		
	    Polyline(hdc, poly, lenof(poly));
	}
#else
...
debug((_T("docursor();")));
...
#if (UNDER_CE > 0 && UNDER_CE < 400)
	{
	    ...
	    poly[0].x = startx;
	    poly[0].y = starty;
	    poly[1].x = startx + dx * length;
	    poly[1].y = starty + dx * length;
...
int char_width(Context ctx, int uc) {
	...
	// Aleq - verify!!!
	{
	    ...
	    mbstowcs(ucw, &c, 1);
	    ucw[1] = _T('\0');
	    DrawText(hdc, ucw, 1, &rect, DT_CALCRECT);
	    ibuf = rect.right - rect.left;
	}
#elif (UNDER_CE >= 400)
	...
	// PPC2000/2002 - GetCharWidth32W result can't be simulated
#else
...
#define SHOW_TOASCII_RESULT
	{			       /* Tell us all about key events */
		...
		if (wParam >= VK_F1 && wParam <= VK_F20)
			...
			break;
		      default:
			debug((_T("VK_%02x"), wParam));
		    }
		if (message == WM_SYSKEYDOWN || message == WM_SYSKEYUP)
		    debug((_T("*")));
		debug((_T(", S%02x"), scan = (HIWORD(lParam) & 0xFF)));
		...
		if (keys[0])
		    debug((_T(", KB0=%02x"), keys[0]));
		if (keys[1])
		    debug((_T(", KB1=%02x"), keys[1]));
		if (keys[2])
		    debug((_T(", KB2=%02x"), keys[2]));
		...
		if (oldstate[ch] != keystate[ch])
		    debug((_T(", M%02x=%02x"), ch, keystate[ch]));
	...
	if (cfg.nethack_keypad && !left_alt) {
	    switch (wParam) {
	      case VK_NUMPAD1:
		*p++ = shift_state ? _T('B') : _T('b');
		return p - output;
	      case VK_NUMPAD2:
		*p++ = shift_state ? _T('J') : _T('j');
		return p - output;
	      case VK_NUMPAD3:
		*p++ = shift_state ? _T('N') : _T('n');
		return p - output;
	      case VK_NUMPAD4:
		*p++ = shift_state ? _T('H') : _T('h');
		return p - output;
	      case VK_NUMPAD5:
		*p++ = shift_state ? _T('.') : _T('.');
		return p - output;
	      case VK_NUMPAD6:
		*p++ = shift_state ? _T('L') : _T('l');
		return p - output;
	      case VK_NUMPAD7:
		*p++ = shift_state ? _T('Y') : _T('y');
		return p - output;
	      case VK_NUMPAD8:
		*p++ = shift_state ? _T('K') : _T('k');
		return p - output;
	      case VK_NUMPAD9:
		*p++ = shift_state ? _T('U') : _T('u');
		return p - output;
	    }
	}
...
void set_title(void *frontend, TCHAR *title)
{
debug((_T("set_title();")));
    sfree(window_name);
    window_name = snewn(1 + _tcsclen(title), TCHAR);
    _tcscpy(window_name, title);
    if (cfg.win_name_always 
#ifndef _WIN32_WCE
	|| !IsIconic(hwnd)
#endif
	)
...
void set_icon(void *frontend, TCHAR *title)
{
debug((_T("set_icon();")));
    sfree(icon_name);
    icon_name = snewn(1 + _tcsclen(title), TCHAR);
    _tcscpy(icon_name, title);
    if (!cfg.win_name_always 
#ifndef _WIN32_WCE
	&& IsIconic(hwnd)
#endif
...
void beep(void *frontend, int mode)
{
...
#ifdef _UNICODE
	wchar_t *wave = ansi2unicode(cfg.bell_wavefile.path);
	...
	if (!PlaySound(cfg.bell_wavefile.path, NULL,
		       SND_ASYNC | SND_FILENAME)) {
#endif
	    TCHAR buf[sizeof(cfg.bell_wavefile) + 80];
	    TCHAR otherbuf[100];
	    _stprintf(buf, _T("Unable to play sound file\n%s\n")
		    _T("Using default sound instead"), cfg.bell_wavefile);
	    _stprintf(otherbuf, _T("%.70s Sound Error"), appname);
 ...
 */
TCHAR *get_window_title(void *frontend, int icon)
{
...
debug((_T("agent_schedule_callback();")));
