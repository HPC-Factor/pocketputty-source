...
#if !(UNDER_CE > 0 && UNDER_CE < 400)
//PPC2000,2002 doesn't support winsock2
...
void sk_init(void)
{
	...
	// ~Aleq debug~
	SOCKET stemp = SOCKET_ERROR;
	if (WSAStartup(MAKEWORD(1, 1), &wsadata) != 0) {
		return;
	}
	stemp = socket(AF_INET, SOCK_STREAM, 0);
	...
	fatalbox(_T("Unable to initialise WinSock"));
    }
    if (LOBYTE(wsadata.wVersion) != LOBYTE(winsock_ver)) {
        p_WSACleanup();
	fatalbox(_T("WinSock version is incompatible with %d.%d"),
...
TCHAR *winsock_error_string(int error)
{
    switch (error) {
      case WSAEACCES:
	return _T("Network error: Permission denied");
      case WSAEADDRINUSE:
	return _T("Network error: Address already in use");
      case WSAEADDRNOTAVAIL:
	return _T("Network error: Cannot assign requested address");
      case WSAEAFNOSUPPORT:
	return
	    _T("Network error: Address family not supported by protocol family");
      case WSAEALREADY:
	return _T("Network error: Operation already in progress");
      case WSAECONNABORTED:
	return _T("Network error: Software caused connection abort");
      case WSAECONNREFUSED:
	return _T("Network error: Connection refused");
      case WSAECONNRESET:
	return _T("Network error: Connection reset by peer");
      case WSAEDESTADDRREQ:
	return _T("Network error: Destination address required");
      case WSAEFAULT:
	return _T("Network error: Bad address");
      case WSAEHOSTDOWN:
	return _T("Network error: Host is down");
      case WSAEHOSTUNREACH:
	return _T("Network error: No route to host");
      case WSAEINPROGRESS:
	return _T("Network error: Operation now in progress");
      case WSAEINTR:
	return _T("Network error: Interrupted function call");
      case WSAEINVAL:
	return _T("Network error: Invalid argument");
      case WSAEISCONN:
	return _T("Network error: Socket is already connected");
      case WSAEMFILE:
	return _T("Network error: Too many open files");
      case WSAEMSGSIZE:
	return _T("Network error: Message too long");
      case WSAENETDOWN:
	return _T("Network error: Network is down");
      case WSAENETRESET:
	return _T("Network error: Network dropped connection on reset");
      case WSAENETUNREACH:
	return _T("Network error: Network is unreachable");
      case WSAENOBUFS:
	return _T("Network error: No buffer space available");
      case WSAENOPROTOOPT:
	return _T("Network error: Bad protocol option");
      case WSAENOTCONN:
	return _T("Network error: Socket is not connected");
      case WSAENOTSOCK:
	return _T("Network error: Socket operation on non-socket");
      case WSAEOPNOTSUPP:
	return _T("Network error: Operation not supported");
      case WSAEPFNOSUPPORT:
	return _T("Network error: Protocol family not supported");
      case WSAEPROCLIM:
	return _T("Network error: Too many processes");
      case WSAEPROTONOSUPPORT:
	return _T("Network error: Protocol not supported");
      case WSAEPROTOTYPE:
	return _T("Network error: Protocol wrong type for socket");
      case WSAESHUTDOWN:
	return _T("Network error: Cannot send after socket shutdown");
      case WSAESOCKTNOSUPPORT:
	return _T("Network error: Socket type not supported");
      case WSAETIMEDOUT:
	return _T("Network error: Connection timed out");
      case WSAEWOULDBLOCK:
	return _T("Network error: Resource temporarily unavailable");
      case WSAEDISCON:
	return _T("Network error: Graceful shutdown in progress");
      default:
	return _T("Unknown network error");
    }
}
...
SockAddr sk_namelookup(const char *host, char **canonicalname)
{
    ...
    *realhost = '\0';
	...
	if (ret->family == 0) {
	    ...
	    ret->error = (err == WSAENETDOWN ? _T("Network is down") :
			  err ==
			  WSAHOST_NOT_FOUND ? _T("Host does not exist") : err
			  == WSATRY_AGAIN ? _T("Host not found") :
#ifdef IPV6
			  fGetAddrInfo ? _T("getaddrinfo: unknown error") :
#endif
			  _T("gethostbyname: unknown error"));
#ifdef DEBUG
	    {
		LPVOID lpMsgBuf;
...
#else
static MIB_IPADDRROW local_addrrow[16];
#endif
...
static int ipv4_is_local_addr(struct in_addr addr)
{
	...
	unsigned long tableSize = sizeof(DWORD) + 16 * sizeof(MIB_IPADDRROW);
    	PMIB_IPADDRTABLE pIpAddrTable = smalloc(tableSize);
	pIpAddrTable->dwNumEntries = 16;
	FillMemory(&pIpAddrTable->table, (unsigned int) tableSize, 0x00);
    
	if (GetIpAddrTable(pIpAddrTable, &tableSize, FALSE) == NO_ERROR) {
	    n_local_interfaces = pIpAddrTable->dwNumEntries;
	    memcpy(&local_addrrow, &pIpAddrTable->dwNumEntries, sizeof(local_addrrow));
	...
	for (i = 0; i < n_local_interfaces; i++) {
	    if (local_addrrow[i].dwAddr == addr.s_addr)
		return 1;	       /* this address is local */
/*	    SOCKADDR_IN *address;
	    address->sin_addr.S_un.S_addr = local_addrrow[i].dwAddr;
...
static const TCHAR *sk_tcp_socket_error(Socket s);

extern TCHAR *do_select(SOCKET skt, int startup);
...
if (len > 0)
{
    TCHAR *wbuf = snewn(len + 1, TCHAR);
    wbuf[len] = _T('\0');
    mbstowcs(wbuf, data, len);
      
_debug_tprintf((_T("send[%s];"), wbuf));
} else
debug((_T("send[n/a];")));
#endif
	...
	if (nsent <= 0) {
		...
		_logtevent(NULL, winsock_error_string(err));
		fatalbox(_T("%s"), winsock_error_string(err));
	    }
	} else {
	    if (s->sending_oob) {
		if (nsent < len) {
...
if (ret > 0)
{
    TCHAR *wbuf = snewn(ret + 1, TCHAR);
    wbuf[ret] = _T('\0');
    mbstowcs(wbuf, buf, ret);
      
_debug_tprintf((_T("recv[%s];"), wbuf));
} else
debug((_T("recv[n/a];")));
#endif
	...
	noise_ultralight(ret);
	if (ret <= 0) {
	    TCHAR *str = (ret == 0 ? _T("Internal networking trouble") :
 ...
 */
const TCHAR *sk_addr_error(SockAddr addr)
{
    return addr->error;
}
static const TCHAR *sk_tcp_socket_error(Socket sock)
{
    Actual_Socket s = (Actual_Socket) sock;
    return s->error;
}
