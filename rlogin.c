...
static int rlogin_closing(Plug plug, const TCHAR *error_msg, int error_code,
			  int calling_back)
{
	...
	_logtevent(rlogin->frontend, error_msg);
	connection_fatal(rlogin->frontend, _T("%s"), error_msg);
 ...
 * freed by the caller.
 */
static const TCHAR *rlogin_init(void *frontend_handle, void **backend_handle,
			       Config *cfg,
