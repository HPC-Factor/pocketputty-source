...
#ifndef _WIN32_WCE // Disabled under Windows CE due to missing time support
	tim = time(NULL);
...
static int x11_closing(Plug plug, const TCHAR *error_msg, int error_code,
		       int calling_back)
{
 ...
 */
const TCHAR *x11_init(Socket * s, char *display, void *c, void *auth,
...
#ifndef _WIN32_WCE
		// Disabled under Windows CE due to missing time
		t = time(NULL);
#endif
